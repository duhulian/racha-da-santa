...
export default function MatchDay() {
  ...
  useEffect(() => { loadMatchDay() }, [])

  async function loadMatchDay() {
    try {
      const { data: matches } = await supabase
        .from('matches').select('*').in('status', ['open', 'sorted'])
      ...
      if (matches && matches.length > 0) {
        ...
        const { data: confs } = await supabase
          .from('confirmations').select('*, players(name, nickname)')
          .eq('match_id', currentMatch.id).order('confirmed_at', { ascending: true })
        setConfirmations(confs || [])
        setIsConfirmed(confs?.some(c => c.player_id === player.id) || false)

        if (currentMatch.status === 'sorted') {
          const { data: teamsData } = await supabase
            .from('teams').select('*, team_players(*, players(name, nickname))')
  ...
  async function toggleConfirmation() {
    if (!match) return
    if (isConfirmed) {
      await supabase.from('confirmations').delete().eq('match_id', match.id).eq('player_id', player.id)
    } else {
      await supabase.from('confirmations').insert({ match_id: match.id, player_id: player.id })
    }
      ...
      {match.status === 'open' && (
        <button onClick={toggleConfirmation}
          className={`w-full py-4 rounded-2xl font-semibold text-lg flex items-center justify-center gap-2 transition ${
            isConfirmed ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-green-600 text-white hover:bg-green-700'
          }`}>
          {isConfirmed ? <><X size={20} /> Cancelar presenca</> : <><Check size={20} /> Confirmar presenca</>}
        </button>
      )}
      ...
      <div className="bg-slate-800 rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="bg-green-500/20 text-green-400 text-xs font-semibold px-2 py-1 rounded-full flex items-center gap-1">
            <Users size={12} /> {confirmations.length}
          </span>
        </div>
          ...
          <div className="space-y-2">
            {confirmations.map((conf, i) => (
              <div key={conf.id} className="flex items-center gap-3 py-2 border-b border-slate-700 last:border-0">
                <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 text-sm font-bold">{i + 1}</div>
                <p className="text-white text-sm font-medium">
                  {conf.players?.nickname || conf.players?.name}
                  {conf.player_id === player.id && <span className="text-green-400 text-xs ml-2">(voce)</span>}
                </p>
              </div>
            ))}
          </div>
          ...
          {teams.map(team => (
            <div key={team.id} className="bg-slate-800 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                {team.won && <span className="bg-yellow-500/20 text-yellow-400 text-xs font-semibold px-2 py-1 rounded-full">Vencedor</span>}
              </div>
              <div className="space-y-1">
                {team.team_players?.map(tp => (
                  <p key={tp.id} className="text-slate-300 text-sm">{tp.players?.nickname || tp.players?.name}</p>
                ))}
              </div>
            </div>
          ))}
