...
const POSITION_CONFIG = {
  goleiro: { label: 'Goleiro', short: 'GOL', color: 'text-gold-400', bg: 'bg-gold-400/10' },
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <button onClick={() => setFilter('all')}
          className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition ${
            filter === 'all' ? 'bg-gold-400 text-navy-900' : 'bg-navy-800 text-slate-400'
          }`}>
          Todos
        </button>
        {Object.entries(POSITION_CONFIG).map(([key, cfg]) => (
          <button key={key} onClick={() => setFilter(key)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition ${
              filter === key ? 'bg-gold-400 text-navy-900' : 'bg-navy-800 text-slate-400'
            }`}>
            {cfg.label}
          </button>
        ))}
