...
export default function Confirm() {
  ...
  async function toggleConfirmation(playerId, currentStatus) {
    ...
    if (currentStatus === 'confirmed') {
      ...
      // Nao respondeu ainda, criar confirmacao
      await supabase
        .from('confirmations')
        .insert({ match_id: match.id, player_id: playerId, status: 'confirmed' })
    }
  ...
  async function declinePlayer(playerId) {
    ...
    if (existing) {
      ...
      await supabase
        .from('confirmations')
        .insert({ match_id: match.id, player_id: playerId, status: 'declined' })
    }
  ...
  async function addAvulso() {
    ...
    // Verificar se ja existe um avulso com esse nome
    const { data: existing } = await supabase
      .from('players')
      .select('id')
      .eq('player_type', 'avulso')
    ...
    if (existing && existing.length > 0) {
      ...
      const { data: newPlayer } = await supabase
        .from('players')
        .insert({ name: trimmedName, player_type: 'avulso', role: 'player' })
        .select()
        .single()
  ...
  if (match.status === 'finished') {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-slate-400 text-sm mt-1">Esse racha ja aconteceu. Confira os resultados no dashboard.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      {/* Info do Racha */}
      <div className="bg-gradient-to-r from-green-900/40 to-green-800/20 border border-green-700/30 rounded-2xl p-4 text-center">
        <p className="text-xs text-green-400 font-semibold uppercase mb-1">Racha Da Santa</p>
      ...
      <div className="bg-slate-800 rounded-2xl p-4">
        <h3 className="text-sm font-semibold text-slate-300 mb-3">Mensalistas</h3>
        <div className="space-y-2">
          {mensalistas.map(p => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-2 py-2 border-b border-slate-700/50 last:border-0">
                <div className="flex-1 min-w-0">
                  <p className="text-white text-sm font-medium truncate">
                    {p.shirt_number ? `${p.shirt_number}. ` : ''}{p.nickname || p.name}
                  </p>
                  {p.position && (
                    <span className="text-xs text-slate-500">{positionLabels[p.position]}</span>
                  )}
                </div>
                <div className="flex gap-1.5">
                  <button
                    onClick={() => toggleConfirmation(p.id, status === 'confirmed' ? 'confirmed' : null)}
                    disabled={isLoading}
      ...
      <div className="bg-slate-800 rounded-2xl p-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-700 rounded-lg p-2.5 text-white outline-none focus:ring-2 focus:ring-green-500 text-sm"
            onKeyDown={(e) => e.key === 'Enter' && addAvulso()}
          />
          <button
            onClick={addAvulso}
            disabled={actionLoading === 'avulso' || !avulsoName.trim()}
            className="bg-green-600 hover:bg-green-700 text-white px-4 rounded-lg transition disabled:opacity-50 text-sm font-semibold"
          >
            ...
            {confirmedList.map((c, i) => (
              <div key={c.id} className="flex items-center gap-2 py-1">
                ...
                {c.players?.player_type === 'avulso' && (
                  <span className="text-xs text-slate-500 bg-slate-700 px-1.5 py-0.5 rounded">avulso</span>
                )}
              </div>
            ))}
          </div>
      ...
      {declinedList.length > 0 && (
        <div className="bg-slate-800 rounded-2xl p-4">
          ...
          <div className="space-y-1.5">
            {declinedList.map(c => (
              <div key={c.id} className="flex items-center gap-2 py-1">
                <div className="w-6 h-6 rounded-full bg-red-500/20 flex items-center justify-center text-red-400 text-xs">
                  <X size={12} />
                </div>
