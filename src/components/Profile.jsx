...
export default function Profile() {
  ...
  async function handleSave() {
    if (!name.trim()) return
    setSaving(true)
    await supabase.from('players').update({ name: name.trim(), nickname: nickname.trim() || null }).eq('id', player.id)
    await refreshPlayer()
    setSaving(false)
    setSaved(true)
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-800 rounded-2xl p-4 space-y-4">
        ...
        <button onClick={handleSave} disabled={saving}
          ...
          <Save size={16} /> {saving ? 'Salvando...' : saved ? 'Salvo!' : 'Salvar alteracoes'}
        </button>
      </div>
